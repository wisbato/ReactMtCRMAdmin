...
const Approveddocument = () => {
  ...
  // Extract users with approved documents
  const userDocuments: UserDocument[] = data?.data ?? [];

  // Filter users who have approved documents
  const approvedUsers = userDocuments.filter((userDoc) => {
    const { documents } = userDoc;
    const hasApprovedDocs = Object.values(documents).some(
  ...
  const sortedAndFilteredUsers = (() => {
    let filteredData = [...approvedUsers];
  ...
  const navigateToApprovedlist = (userId: number) => {
    navigate(`/approvedrequestlist/${userId}`);
  };
  ...
  const exportToCSV = () => {
    ...
    a.setAttribute("download", "approved_document_users.csv");
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
      <head>
          <title>Approved Document List - Print</title>
          <style>
              body {
  ...
  // Loading state
  if (isLoading) {
    return <div className="loading">Loading approved documents...</div>;
  }

  // Error state
  if (isError && error instanceof Error) {
    return (
      <div className="error">Error loading approved documents: {error.message}</div>
    );
  }

  return (
    <div className={`user-list-main ${theme === "dark" ? "dark-mode" : ""}`}>
      <h1 className="fw-bold" style={{ color: "var(--primary-color)" }}>Approved Document List</h1>
      ...
      <div className={`userlist-container ${theme === "dark" ? "dark-mode" : ""}`}>
        <div className="search-section d-flex flex-column flex-md-row justify-content-between align-items-stretch align-items-md-center mb-3 gap-2 gap-md-0">
          <Form.Group as={Col} md="4" lg="3">
            <Form.Control
              className={`userlist-inputbox ${theme === "dark" ? "dark-mode" : ""}`}
              type="text"
              placeholder="Search..."
              value={searchTerm}
              onChange={(e) => {
          ...
          <table className="table caption-top table-hover">
            <thead className={`table-light ${theme === "dark" ? "dark-mode" : ""}`}>
              <tr>
                ...
                <th className={theme === "dark" ? "dark-mode-th" : ""}>Action</th>
              </tr>
            </thead>
              ...
              {currentUsers.length > 0 ? (
                currentUsers.map((userDoc) => (
                  ...
                  >
                    ...
                    <td
                      className={`column-container ${theme === "dark" ? "dark-mode-td" : ""}`}
                      onClick={() => navigateToApprovedlist(userDoc.user.id)}
                ...
                <tr className={theme === "dark" ? "dark-mode-tr" : ""}>
                  <td
                    colSpan={6}
                    className={`text-center ${theme === "dark" ? "dark-mode-td" : ""}`}
                  >
                    No users found with approved documents.
                  </td>
                </tr>
              )}
            </tbody>
