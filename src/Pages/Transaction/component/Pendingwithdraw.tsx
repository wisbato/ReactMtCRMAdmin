...
import { getWithdrawal } from '../../../api/pending_transaction/getWithdrawal';
import { approveWithdrawal, rejectWithdrawal } from '../../../api/pending_transaction/app_rejWithdr';
...
const Pendingwithdraw = () => {
  ...
  // Fetch withdrawals using useQuery
  const {
  ...
  // Sorting function
  const requestSort = (key: keyof UserWithdrawal) => {
  ...
  const getSortIcon = (columnKey: keyof UserWithdrawal) => {
  ...
  const filteredAndSortedWithdrawals = (() => {
    ...
    if (searchTerm) {
      filteredData = filteredData.filter((withdrawal) =>
        Object.values(withdrawal).some((value) =>
  ...
  const currentWithdrawals = filteredAndSortedWithdrawals.slice(
    indexOfFirstEntry,
    indexOfLastEntry
  );
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(['withdrawals'], (oldData: any) => {
        if (!oldData?.withdrawals) return oldData;
        return {
          ...oldData,
          withdrawals: oldData.withdrawals.map((withdrawal: any) => 
            withdrawal.id === data.withdrawal.id 
              ? { ...withdrawal, comment: 'Approved', status: 'approved' } 
              : withdrawal
          )
        };
      });
  ...
  // Reject mutation - fixed to properly handle comments
  const rejectMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Update the local comment field with the rejection reason
      queryClient.setQueryData(['withdrawals'], (oldData: any) => {
        if (!oldData?.withdrawals) return oldData;
        return {
          ...oldData,
          withdrawals: oldData.withdrawals.map((withdrawal: any) => 
            withdrawal.id === data.withdrawal.id 
              ? { 
                  ...withdrawal, 
                  comment: data.withdrawal.comment || rejectComment,
                  status: 'rejected' 
                } 
              : withdrawal
          )
  ...
  const exportToCSV = () => {
    ...
    currentWithdrawals.forEach((withdrawal) => {
      const row = [
        ...
        withdrawal.bankAccount ? `"${withdrawal.bankAccount}"` : "-",
    ...
    a.setAttribute("download", "pending_withdrawals.csv");
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
      <head>
          <title>Pending Withdrawals - Print</title>
          <style>
              body {
      ...
      <body>
          <h1>Pending Withdrawals Report</h1>
  ...
  return (
    <div className='user-list-main'>
      <h1 className="fw-bold">Pending Withdrawal</h1>
      ...
      <div className={`userlist-container ${theme === "dark" ? "bg-dark" : ""}`}>
          ...
          <table className="table caption-top table-hover">
            <thead className={`table-light ${theme === "dark" ? "dark-mode" : ""}`}>
              <tr>
                {columns.map((column) => (
                  <th
                    key={column.key}
                    onClick={() => column.key !== "action" && requestSort(column.key as keyof UserWithdrawal)}
                    style={{ cursor: column.key !== "action" ? "pointer" : "default" }}
                  >
                    {column.label}
                    {column.key !== "action" && (
                      <span style={{ marginLeft: "8px" }}>
                        {getSortIcon(column.key as keyof UserWithdrawal)}
                      </span>
                    )}
                  </th>
                ))}
            ...
            <tbody>
              {currentWithdrawals.length > 0 ? (
                currentWithdrawals.map((withdrawal, index) => (
                  <tr key={withdrawal.id} className={theme === "dark" ? "dark-mode-tr" : ""}>
                    ...
                    <td className={theme === "dark" ? "dark-mode-td" : ""}>
                      {withdrawal.creator.name} <br />
                      <small>{withdrawal.creator.email}</small>
                    </td>
                    ...
                    {/* Note column - shows the original note */}
                    <td className={`text-truncate max-width-150 {theme === "dark" ? "dark-mode-td" : ""}`}>
                      {withdrawal.note || '-'}
                    </td>
                    {/* Comment column - shows approval/rejection comments */}
                    <td className={`text-truncate max-width-150 ${theme === "dark" ? "dark-mode-td" : ""}`}>
                      {withdrawal.comment || "-"}
                    </td>
                    <td className={theme === "dark" ? "dark-mode-td" : ""}>
                      <span className={`badge ${
                        ...
                        withdrawal.status === "approved" ? "bg-success" : "bg-danger"
                      }`}>
                        {withdrawal.status.charAt(0).toUpperCase() + withdrawal.status.slice(1)}
                      </span>
                    </td>
                    <td className={theme === "dark" ? "dark-mode-td" : ""}>{new Date(withdrawal.createdAt).toLocaleString()}</td>
                    <td className={theme === "dark" ? "dark-mode-td" : ""}>
                ...
                <tr>
                  <td colSpan={columns.length} style={{ textAlign: 'center', padding: '20px' }} className={theme === "dark" ? "dark-mode-td" : ""}>
        ...
        <div className='pagination-container'>
          <div className='table-bottom-content'>
            <span>
              Showing {indexOfFirstEntry + 1} to {Math.min(indexOfLastEntry, filteredAndSortedWithdrawals.length)} of{" "}
